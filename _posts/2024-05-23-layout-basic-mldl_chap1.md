...
* Scikit-learn: 파이썬 API를 사용한 머신러닝 라이브러리. 사이킷런 라이브러리에서 제공하는 클래스와 함수로 편하게 사용, 그러나 신규 머신러닝 알고리즘이 빠르게 업데이트 되지는 않고, 충분히 검증된 뒤 추가됨. *이 책의 1장부터 6장까지 배울 머신러닝 알고리즘은 사이킷런에 포함된 알고리즘들임*<br><br>
